...
for row in csv_obj:     
    ...
    new_list.append([fnam,lnam,email, nmls])
input_file.close()
#print(new_list)
...
entries = set()
dup = [['First Name','Last Name','Email','NMLS']]
for row in new_list:
    key = (row[0], row[1],row[3]) # instead of just the last name
    if key not in entries:
        entries.add(key)
    else:
        dup.append([row[0],row[1],row[2],row[3]])
